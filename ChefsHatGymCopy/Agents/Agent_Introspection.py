...
class DQL(IAgent.IAgent):
    ...
    def getAction(self, observations):
        ...
        if np.max(a) != 1:
            if self.Qvalues[0,(np.argmax(a))] < np.max(a):
                self.Qvalues[0,(np.argmax(a))] = round(np.max(a), 4)
    ...
    def probability(self):
        rewCalculate = 1 + (self.sumRewards * -0.001)
        print(self.Qvalues[0])
        probability = self.pSuccess(self.Qvalues[0], rewCalculate, 0.99)
